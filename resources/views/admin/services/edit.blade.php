    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.services.update", [$service->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" name="address" id="address" required>{{ old('address', $service->address) }}</textarea>
            ...
            <div class="form-group">
                <label for="type">{{ trans('cruds.service.fields.type') }}</label>
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $service->type) }}">
                @if($errors->has('type'))
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.service.fields.status') }}</label>
            ...
            <div class="form-group">
                <label class="required" for="users">{{ trans('cruds.service.fields.user') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('users') ? 'is-invalid' : '' }}" name="users[]" id="users" multiple required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (in_array($id, old('users', [])) || $service->users->contains($id)) ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('users'))
