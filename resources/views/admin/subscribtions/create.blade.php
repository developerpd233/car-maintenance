...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.subscribtion.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.subscribtions.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\Subscribtion::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'not_paid') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
